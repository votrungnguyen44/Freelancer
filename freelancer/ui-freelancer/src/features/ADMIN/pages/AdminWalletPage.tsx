...
import {
  WalletOutlined, HistoryOutlined, ArrowDownOutlined,
...
const incomeColumns: ColumnsType<MyIncomeTransactionResponse> = [
  {
    ...
    render: (v, r) => (
      <div>
        <Text strong style={{ fontSize: 13 }}>{v || r.projectName || "—"}</Text>
      </div>
    ),
  },
  {
    ...
    key: "amount",
    render: (v) => (
      <Text style={{ color: "#52c41a", fontWeight: 600 }}>+{fmt(v)}</Text>
    ),
...
const withdrawColumns: ColumnsType<WithdrawHistoryResponse> = [
  ...
  },
  {
    ...
    dataIndex: "amount",
    key: "amount",
    render: (v) => <Text style={{ color: "#ff4d4f", fontWeight: 600 }}>-{fmt(v)}</Text>,
    sorter: (a, b) => a.amount - b.amount,
  },
  {
    title: "Trạng thái",
...
export const AdminWalletPage: React.FC = () => {
  ...
  const kpis = [
    { title: "Số dư ví", value: fmt(wallet?.balance ?? 0), icon: <WalletOutlined />, color: "#1677ff" },
    { title: "Tổng thu nhập", value: fmt(totalIncome), icon: <DollarOutlined />, color: "#52c41a" },
    { title: "Đã rút", value: fmt(totalWithdrawn), icon: <CheckCircleOutlined />, color: "#722ed1" },
    { title: "Đang xử lý", value: fmt(pendingWithdraw), icon: <ClockCircleOutlined />, color: "#faad14" },
  ];
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {kpis.map((s, i) => (
          <Col span={6} key={i}>
            <Card
              bordered={false}
              loading={walletLoading || incomeLoading || withdrawLoading}
      ...
      >
        {wallet && (
          <div style={{
            ...
            <Text strong style={{ fontSize: 18, color: "#1677ff" }}>{fmt(wallet.balance)}</Text>
          </div>
        )}
        <Form form={form} layout="vertical" onFinish={handleWithdraw}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <Button
              ...
              htmlType="submit"
              loading={submitting}
              style={{ background: "linear-gradient(135deg, #1677ff, #6366f1)", border: "none", borderRadius: 8 }}
            >
              Xác nhận rút tiền
            </Button>
          </div>
